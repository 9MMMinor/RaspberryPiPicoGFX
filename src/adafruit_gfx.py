...
class GFX:
    ...
    def __init__(
        ...
        if text is None:
            ...
            if font is None:
                ...
                self.font = std_font
                self.set_text_background(0x867D,0)
            else:
                self.font = font
    ...
    def _place_char(self, x0, y0, char, size, *args, **kwargs):
        ...
        Modified (MMM) for lower left origin coordinates
        """
...
#                            size * (height - y - 1) + y0,
    ...
    # Xbm files are monochrome bitmaps
    # Draw XBitMap Files (*.xbm), exported from GIMP,
    # Usage: Export from GIMP to *.xbm, rename *.xbm to *.py and open in editor.
    # Change C Array to python array.array('B',[]) and name it
    # Create a bitmapTuple = (bitmap, width, height) where width and height
    # are the dimensions of the bitmap in pixels.
    def drawBitmap(self, x, y, bitmapTuple, color, bg=0, transparent=True, Xbit=True):
        '''Draw XBitMap Files (*.xbm), exported from GIMP
        
        '''
        (bitmap,w,h) = bitmapTuple
        self.drawXBitmap(x,y,bitmap,w,h,color,bg,transparent,Xbit)


    def drawXBitmap(self, x, y, bitmap, w, h, color, bg, transparent, Xbit):
        bo = 0
        bits = 0
        bit = 0
        if Xbit: bitMask = 0x01
        else: bitMask = 0x80
        ...
        for yy in range(h):
            for xx in range(w):
                ...
                y1 = h + y - yy
                if(bits & bitMask):
                    self._pixel(x+xx, y1, color)
                elif not transparent:
                    self._pixel(x+xx, y1, bg)
                if Xbit: bits >>= 1
                else: bits <<= 1
